...
**Implemented `64053fe`.** Borrows the NGF shape exactly as scoped: observer-requested ("REQUEST FIRE UNIT STATUS" / "FIRE UNIT STATUS, OVER", STT-tolerant), answered **once** with round count, real battery grid, tubes and munitions, closed with "ON STATION AND READY FOR CALL FOR FIRE." No readback; a second request gets a one-line brush-off rather than the full reply again. Reply is per-asset (six-gun 155 battery vs. two-tube 60mm section; G17's HACKSAW delivery swap applies on mortar chapters). Round counts are flavor state, not a real ammunition-tracking model. The in-mission MTO is untouched, per the user's explicit constraint. Verified 9/9 live e2e (classification including bare-FIRE/CFF non-interference, full first reply, on-station close, brush-off second reply); parser-order guard clean; replay over 247 recorded transmissions unchanged; boots clean offline.
...
**12h — shell selection (smoke/illumination).** Shell is a method-of-engagement element like sheaf/fuze: named in the call (SHELL SMOKE / WP → smoke / ILLUMINATION), switched bare mid-mission ("SHELL SMOKE, OVER"); immediate smoke rides the same one-transmission path as immediate suppression (mission-types line above). The MTO announces the real nature (155 SMOKE / 155 ILLUMINATION FUZE TIME) rather than the shell words verbatim, which are stripped from the description readback.

**Implemented `6b7fd3c`.** SMOKE pools standing screens (~90 s), scoring ZERO casualties — only HE feeds the graded model, since smoke cannot kill convoy vehicles — but a screen within `EFFECTS.screen.radius` (120 m) of the enemy suppresses by OBSCURATION: intent follows the shell, so a screening mission still grades PASS — TARGET SUPPRESSED. ILLUMINATION never touches ground: no crater/casualties/fratricide/alerts — one pooled PointLight + flare sprite rides down over ~55 s (r160's physical light units needed ~240k candela to pool light on terrain 300 m below; a first attempt at 2.8 legacy units lit nothing, caught by the night screenshot). Graded PASS — ILLUMINATION PROVIDED; the strict net doesn't demand casualty BDA for a mission whose whole point is light. Verified 11/11 e2e (parse aliases, live immediate-smoke mission with obscuration-suppression PASS, live night illum mission with a screenshot of the lit terrain pool); 12/12 shots; effects harness 48/48 unchanged; replay over 247 unchanged.
