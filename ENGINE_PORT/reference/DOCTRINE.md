> **PORT REFERENCE — transfers unchanged.** This document is engine-agnostic: it specifies doctrine, not implementation. Source of truth today is `..\..\DOCTRINE.md` in the browser-sim repo; when the port becomes its own repository this copy becomes canonical. Do not let the two drift — change one, change the other.
