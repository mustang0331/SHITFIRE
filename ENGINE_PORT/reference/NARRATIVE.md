> **PORT REFERENCE — transfers unchanged.** This document is engine-agnostic: it specifies campaign content, not implementation. Source of truth today is `..\..\NARRATIVE.md` in the browser-sim repo; when the port becomes its own repository this copy becomes canonical. Do not let the two drift — change one, change the other.
