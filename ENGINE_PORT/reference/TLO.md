> **PORT REFERENCE — transfers unchanged.** This document is engine-agnostic: it specifies training objectives, not implementation. Source of truth today is `..\..\TLO.md` in the browser-sim repo; when the port becomes its own repository this copy becomes canonical. Do not let the two drift — change one, change the other.
...
- Adjust fire onto the target and achieve fire for effect within **4 adjusting rounds** — the sim's own applied par (`passMaxAdjustRounds` in `CONFIG`), deliberately looser than the raw JFO-SSUP-2001 figure of ≤3 rounds cited in `DOCTRINE.md`, matching this sim's more forgiving training pace.
