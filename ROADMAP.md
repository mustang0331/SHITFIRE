...
**Next:** build the researched cluster — **G13** (graded destroyed/neutralized/suppressed outcomes + continue-after-suppression), then **G18** (per-asset radii off the researched figures), **G15/G16** (sheaf/fuze), **G10** (FIRE UNIT STATUS exchange, borrowed NGF shape per research). Research is on file in DOCTRINE.md §Pre-mission and effects data, `8304ef6`.
...
| G13 | **Effects criteria and casualty radii are probably too small.** Needs **destroyed / neutralized / suppressed** as distinct outcomes with distinct criteria, per JFIRE. Give the observer the option to **continue the mission if the target is only suppressed**. **Finding: the existing ≥10%/≥30% figures are confirmed against FM 6-30; the only sourced rounds-to-effect table is 60mm/5-rounds-per-tube (posture-dependent), nothing found bars continue-after-suppression.** See [DOCTRINE.md](DOCTRINE.md#pre-mission-and-effects-data-researched-2026-07-29-for-g10g13g15g16g18). | Opus | Three graded outcomes with sourced radii/criteria; "suppressed" offers continue-or-end; `effectRadius`/`hitsToNeutralize` replaced by the graded model | RESEARCHED — build ready |
...
| G18 | **60mm and artillery need different effective radii.** A mortar round and a 155 do not do the same thing. Research required. **Finding: sourced RED (safety) tables exist for both (JFIRE App. F); true lethal-area figures are classified (JMEM/GMET) — best unclassified proxies are the 60mm suppression-radius/FPF-width figures, flagged ⚠ for 155mm.** See [DOCTRINE.md](DOCTRINE.md#pre-mission-and-effects-data-researched-2026-07-29-for-g10g13g15g16g18). | Opus | Per-asset effect radius sourced and recorded; interacts with G13 — **land G13 first** | RESEARCHED — build ready |
