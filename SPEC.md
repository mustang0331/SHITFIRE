...
> **Stage numbers are stable identifiers, not the shipping order.** They map to commit messages and, in one case, to a string inside the code ([index.html:3691](index.html#L3691) carries `blurb: 'stage 11'`), so they are never renumbered. **[ROADMAP.md](ROADMAP.md) is the single authority on what ships next and what is done** — as of 2026-07-29 that order is stage 13 → the stage 12 remainder → stage 11. This section defines *scope per stage*; it does not define sequence.
