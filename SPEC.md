...
> **Stage numbers are stable identifiers, not the shipping order.** They map to commit messages and, in one case, to a string inside the code (search `blurb: 'stage 11'` in SHITFIRE.html), so they are never renumbered. **[ROADMAP.md](ROADMAP.md) is the single authority on what ships next and what is done** — as of 2026-07-29 that order is stage 13 → the stage 12 remainder → stage 11. This section defines *scope per stage*; it does not define sequence.
...
12. **FO skill depth & training fidelity.** A session-transcript review found the sim grades outcomes without teaching the two core FO skills it exists to train — target location and adjustment doctrine. Close the gap:
    ...
    - **Dynamic environment.** Replace the permanent-noon, no-wind default. ⚠ **Time of day is NOT built here — stage 13 owns it** (it is the lighting authority: sun, hemisphere, and fog all derive from one TOD value, so implementing it twice would fork the renderer). Stage 12 builds only what *consumes* TOD: a **wind** model that drifts smoke (making it readable as a tool, not just an effect), **night/illumination missions**, and **degraded visibility**, so observation is not uniformly easy.

13. **Visual overhaul.** The renderer's three named defects — 33 m terrain facets larger than the 60 m effect radius, adaptive quality silently making targets unresolvable through binos, and no relief shading for terrain association — plus the beautification that rides along with fixing them: tone mapping, a real sky and **time-of-day model**, baked hillshade/AO in terrain vertex colors, a near-field LOD patch, instanced vegetation, shoreline and water, persistent craters, and optics presentation. Every item is gated behind one `CONFIG.GFX` flag so it can be A/B'd or reverted alone. **Full detail spec: [GRAPHICS.md](GRAPHICS.md).** Governing rule for the whole stage: *this is a training instrument, not a demo* — any change that reduces the observer's ability to resolve a target at 1500–3200 m, tell a civilian element from a military one, or see exactly where a round landed is a **regression even if it looks better**. No `EffectComposer` on the default path, no PBR conversion, no cascaded shadow maps, no external assets.
14. **Dialogue punch-up.** Apply the rewritten HELLHOUND/GUNNY quip pools, chapter narrative beats, and mission briefs from a session-transcript review that caught the FDC repeating itself. Content only — no mechanics change, and **never at the cost of the doctrinal readback**. Full detail spec: [DIALOGUE_REVISIONS.md](DIALOGUE_REVISIONS.md).
...
Make it feel incredible before it feels realistic: the first time a player SPEAKS "HELLHOUND FIRES, this is MUSTANG 12, adjust fire, over" and a dry, slack-jawed voice drawls back "MUSTANG 12, HELLHOUND FIRES — send it, and try to hit the right island this time, over," then watches a round land 120 meters off and calls "right 50, add 100" and walks it onto the target — that is the moment. One `SHITFIRE.html`, no keys, runs on a barracks laptop in a browser; the only optional external input is a supplied island heightmap, everything else hand-written except Three.js. If a feature is missing, the trainer should still teach the loop: observe → locate → transmit → observe impact → correct → fire for effect → assess.
